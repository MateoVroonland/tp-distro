...
func startServer() {
	...
	for filesRemaining > 0 {
		...
		message, err := handleConnection(conn, file)
		if err != nil {
		...
		case "credits":
			log.Printf("Received %d bytes of credits", len(message))
			credits += string(message)
		case "ratings":
			log.Printf("Received %d bytes of ratings", len(message))
...
func handleConnection(conn net.Conn, file string) (string, error) {
	defer conn.Close()
